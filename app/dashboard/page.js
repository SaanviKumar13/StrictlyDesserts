      ...
      <div className="col-span-12 bg-white rounded-xl shadow-md p-6 border border-pink-100">
        ...
        {chartData.length > 0 ? (
          ...
          <div className="h-96 flex items-center justify-center">
            <p className="text-black">No forecast data available</p>
          </div>
        )}
      </div>
    );
    ...
    <div className="col-span-12 bg-white rounded-xl shadow-md p-6 border border-pink-100">
      ...
      {topItems.length > 0 ? (
        ...
        <div className="h-64 flex items-center justify-center">
          <p className="text-black">No product data available</p>
        </div>
      )}
    </div>
  );
  ...
  const ForecastTab = () => (
    <div className="col-span-12 bg-white rounded-xl shadow-md p-6 border border-pink-100">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-sm text-black bg-pink-100 px-3 py-1 rounded-full">
          Next {forecastData.length} Days
        </div>
      </div>
      {forecastData.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-pink-200">
            <thead>
              <tr>
                <th className="text-left text-sm font-semibold text-pink-800 pb-3">Date</th>
                <th className="text-right text-sm font-semibold text-pink-800 pb-3">Forecast</th>
                <th className="text-right text-sm font-semibold text-pink-800 pb-3">Min</th>
                <th className="text-right text-sm font-semibold text-pink-800 pb-3">Max</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-pink-100">
              {forecastData.map((item, index) => (
                <tr key={index}>
                  <td className="py-3 text-sm font-medium text-black">{item.date}</td>
                  <td className="py-3 text-right text-sm text-black">{Number(item.forecast).toFixed(2)}</td>
                  <td className="py-3 text-right text-sm text-black">{Number(item.lower).toFixed(2)}</td>
                  <td className="py-3 text-right text-sm text-black">{Number(item.upper).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="bg-pink-50 min-h-screen font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex flex-wrap gap-2 mb-6">
          <button 
            className={`px-4 py-2 rounded-lg transition-all ${activeTab === 'overview' ? 'bg-pink-500 text-white shadow-md' : 'bg-white text-black hover:bg-pink-100'}`}
            onClick={() => setActiveTab('overview')}
          >
            Dashboard
          </button>
          <button 
            className={`px-4 py-2 rounded-lg transition-all ${activeTab === 'products' ? 'bg-pink-500 text-white shadow-md' : 'bg-white text-black hover:bg-pink-100'}`}
            onClick={() => setActiveTab('products')}
          >
            Products
          </button>
          <button 
            className={`px-4 py-2 rounded-lg transition-all ${activeTab === 'forecast' ? 'bg-pink-500 text-white shadow-md' : 'bg-white text-black hover:bg-pink-100'}`}
